  ...
  try {
    const recommendationsSchema = z.object({
      ...
      preferences: z.object({
        ...
        minPrice: z.number().min(0).max(1000000000000).optional(),
        maxPrice: z.number().min(0).max(1000000000000).optional(),
        bedrooms: z.number().min(0).max(50).optional(),
      }).optional(),
    });

    const body = await req.json();
    const validationResult = recommendationsSchema.safeParse(body);
